...
# # s_logn_quant_lin_regression [<img src="https://www.arpm.co/lab/icons/icon_permalink.png" width=30 height=30 style="display: inline;">](https://www.arpm.co/lab/redirect.php?code=s_logn_quant_lin_regression&codeLang=Python)
# For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=s_logn_quant_lin_regression).
...
# ## [Input parameters](https://www.arpm.co/lab/redirect.php?permalink=s_logn_quant_lin_regression-parameters)
...
# ## [Step 1](https://www.arpm.co/lab/redirect.php?permalink=s_logn_quant_lin_regression-implementation-step01): Generate samples
...
# ## [Step 2](https://www.arpm.co/lab/redirect.php?permalink=s_logn_quant_lin_regression-implementation-step02): Compute optimal/arbitrary predictor, prediction and residuals

# +
data = pd.DataFrame({'outputs': x, 'inputs': z})
model = smf.quantreg('outputs ~ inputs', data).fit(q=0.5)

alpha = model.params['Intercept']
beta = model.params['inputs']
...
# ## [Step 3](https://www.arpm.co/lab/redirect.php?permalink=s_logn_quant_lin_regression-implementation-step03): Expectation and covariance of (Xhat, U)
...
# ## [Step 4](https://www.arpm.co/lab/redirect.php?permalink=s_logn_quant_lin_regression-implementation-step04): expectation-covariance ellipsoid computations
...
# +
# Compute the distances from arb. point to linear optimal and arbitrary prediction
...
chi_arb_select1 = chi_arb(z[index1])
chi_arb_select2 = chi_arb(z[index2])
...
ax1.plot([z[index1], z[index1]], [x[index1], chi_arb_select1],
         lw=12, color='b', alpha=0.4)
ax1.plot([z[index2], z[index2]], [x[index2], chi_arb_select2],
         lw=12, color='b', alpha=0.4)
ax1.plot([z[index1], z[index1]], [x[index1], chi_z_val_select1],
         lw=4, color=orange)
ax1.plot([z[index2], z[index2]], [x[index2], chi_z_val_select2],
         lw=4, color=orange)
ax1.scatter([z[index1], z[index2], z[index1], z[index2], z[index1], z[index2]],
            [x[index1], x[index2], chi_z_val_select1, chi_z_val_select2,
             chi_arb_select1, chi_arb_select2],
