...
# # s_call_quant_lin_regression [<img src="https://www.arpm.co/lab/icons/icon_permalink.png" width=30 height=30 style="display: inline;">](https://www.arpm.co/lab/redirect.php?code=s_call_quant_lin_regression&codeLang=Python)
# For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=s_call_quant_lin_regression).
...
# ## [Input parameters](https://www.arpm.co/lab/redirect.php?permalink=s_call_quant_lin_regression-parameters)

c = 0.5  # quantile level
delta_t = 100  # horizon parameter

# ## [Step 0](https://www.arpm.co/lab/redirect.php?permalink=s_call_quant_lin_regression-implementation-step00): Load data
...
# load from the database db_implvol_optionSPX
...
# ## [Step 1](https://www.arpm.co/lab/redirect.php?permalink=s_call_quant_lin_regression-implementation-step01): Compute returns
...
# ## [Step 2](https://www.arpm.co/lab/redirect.php?permalink=s_call_quant_lin_regression-implementation-step02): Approximate conditional median Med{X|z} with linear function

# +
df = pd.DataFrame({'outputs': r_call, 'inputs': r_sandp})
regression = smf.quantreg('outputs ~ inputs', df).fit(q=c)
del df
# parameters of the linear quantile regression predictor
alpha = regression.params['Intercept']
beta = regression.params['inputs']

# linear quantile regression predictor
def chi_alpha_beta(z):
    return alpha + beta*np.array(z)
# -

# ## [Step 3](https://www.arpm.co/lab/redirect.php?permalink=s_call_quant_lin_regression-implementation-step03): Evaluate prediction and residuals

r_bar_call = chi_alpha_beta(r_sandp)  # prediction
u = r_call-r_bar_call  # residuals

# ## [Step 4](https://www.arpm.co/lab/redirect.php?permalink=s_call_quant_lin_regression-implementation-step04): Compute payoff at t_end and current value of the call option as a function of the underlying S&P500
...
plt.annotate(r'$\bf{quantile}$: $' + str(round(c, 2)) + r'$', (0.23, 0.78),
             xycoords='figure fraction', size=16, color=orange)
ax1.set_title('Linear quantile regression',
...
if c == 0.5:
    string = 'LAD lin. approx.'
else:
    string = 'Quantile lin. approx.'
fig.legend((l1, l2, l3, l4, l5, l6, l9, l7),
           ('Input', 'Output', 'Predictor', 'Residual', string,
