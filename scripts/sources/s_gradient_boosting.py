...
max_leaves = 80
min_boost = 20
max_boost = 101  # the boosting runs from min_boost to min_boost + max_boost
n_sample = 1000
mu_z = np.zeros(2)  # expectation
sigma2_z = np.array([[1, 0], [0, 1]])  # covariance

# ## [Step 1](https://www.arpm.co/lab/redirect.php?permalink=s_gradient_boosting-implementation-step01): Define features and target variables
...
# ## [Step 2](https://www.arpm.co/lab/redirect.php?permalink=s_gradient_boosting-implementation-step02): Decision tree regressor with gradient boosting

err_in = np.zeros(max_boost)
for boost in np.arange(min_boost, min_boost+max_boost):
    boost_reg = GradientBoostingRegressor(max_depth=max_depth,
                                          max_leaf_nodes=max_leaves,
                                          n_estimators=boost)
    err_in[boost-min_boost] = np.mean((x-boost_reg.fit(z, x).predict(z))**2)
...
insamplot = ax1.plot(np.arange(max_boost)+1, err_in, color='k')
ax1.set_ylabel('In-sample error', color='k')
ax1.tick_params(axis='y', colors='k')
ax1.set_xlabel('Boosting')
plt.xlim([0, max_boost + 1])
