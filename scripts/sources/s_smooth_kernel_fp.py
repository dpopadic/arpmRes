...
# # s_smooth_kernel_fp [<img src="https://www.arpm.co/lab/icons/icon_permalink.png" width=30 height=30 style="display: inline;">](https://www.arpm.co/lab/redirect.php?code=s_smooth_kernel_fp&codeLang=Python)
# For details, see [here](https://www.arpm.co/lab/redirect.php?permalink=ExerKernStateCond).
...
from arpym.estimation import effective_num_scenarios, smooth_kernel_fp
from arpym.statistics import scoring, smoothing
from arpym.tools import colormap_fp, histogram_sp, add_logo
# -

# ## [Input parameters](https://www.arpm.co/lab/redirect.php?permalink=s_smooth_kernel_fp-parameters)

z_star = 1  # target value
h = 0.2  # kernel bandwidth
gamma = 2  # kernel type parameter

# ## [Step 0](https://www.arpm.co/lab/redirect.php?permalink=s_smooth_kernel_fp-implementation-step00): Upload data
...
# smooth kernel probabilities plot
fig2, axs = plt.subplots(2, 1, figsize=(1280.0/72.0, 720.0/72.0), dpi=72.0)
axs[0].bar(plot_dates[1:], p_smooth, color='gray',
           width=np.floor(len(p_smooth)/680))
for label in axs[0].xaxis.get_ticklabels():
    label.set_fontsize(14)
axs[0].set_yticks([])
axs[0].set_xlim(min(plot_dates), max(plot_dates))
axs[0].xaxis.set_major_formatter(myFmt)
axs[0].set_ylim(0, np.max(p_smooth)*(1+1./7.))
axs[0].set_ylabel('probability', fontsize=17)
plt.yticks(fontsize=14)
axs[0].set_title('Smooth kernel conditioning probabilities',
